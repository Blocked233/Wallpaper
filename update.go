...
func update() {
	...
	for {
		...
		for _, val := range jsonmsg.Images {
			...
			qiniu.Upload2Qiniu("msbingwallpaper", val.Enddate+".jpg", bingURL+val.Urlbase+"_UHD.jpg")
			upload(val)
...
func upload(val Image) {
	...
	// Upload to redis
	cache.RedisClient.Set(item.ID, item, 24*31*time.Hour)
...
func updateMonth(t time.Time, wallpaperParams *wallpaperParams) {
	year, month, _ := t.Date()
...
func getWallpapers(t time.Time) []cosmosdb.WallpaperItem {
	results := []cosmosdb.WallpaperItem{}

	year, month, _ := t.Date()
	firstDayOfMonth := time.Date(year, month, 1, 0, 0, 0, 0, time.Local)
	for i := t.Day() - 1; i >= 0; i-- {
		d := firstDayOfMonth.AddDate(0, 0, i)
		b, err := cache.Wallpaper.Get(d.Format("20060102"))
		if b == nil || err != nil {
			return results
		}
